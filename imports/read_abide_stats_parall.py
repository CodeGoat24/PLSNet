...
def read_sigle_data(data_dir,filename,use_gdc =False):
    ...
    # # 获取时间序列
    # timeseries = get_timeseries_by_ID(filename[:5])
    ...
    if use_gdc:
        ...
        # 增加了时间序列
        label = np.append(label,int(filename[:5]))
        return edge_att.data.numpy(),edge_index.data.numpy(),att,label,num_nodes



if __name__ == "__main__":
    data_dir = '/home/azureuser/projects/net/data/ABIDE_pcp/cpac/filt_noglobal/raw'
    filename = '50346.h5'
