...
# root_folder = '../data'
root_folder = '/data/CodeGoat24/data'

# data_folder = os.path.join(root_folder, 'ABIDE_pcp/cpac/filt_noglobal').replace('\\', '/')
data_folder = '/data/CodeGoat24/data/ABIDE_pcp/cpac/filt_noglobal'
phenotype = '/data/CodeGoat24/data/ABIDE_pcp/Phenotypic_V1_0b_preprocessed1.csv'
# phenotype = os.path.join(root_folder, 'ABIDE_pcp/Phenotypic_V1_0b_preprocessed1.csv').replace('\\', '/')
...
def get_timeseries(subject_list, atlas_name="cc200", silence=True, data_folder = '/data/CodeGoat24/data/ABIDE_pcp/cpac/filt_noglobal'):
    """
        subject_list : list of short subject IDs in string format
    ...
    for i in range(len(subject_list)):
        subject_folder = os.path.join(data_folder, str(subject_list[i]))
        ro_file = [f for f in os.listdir(subject_folder) if f.endswith('_rois_' + atlas_name + '.1D')]
        fl = os.path.join(subject_folder, ro_file[0])
        if silence != True:
            print("Reading timeseries file %s" % fl)
        t = np.loadtxt(fl, skiprows=0).transpose(1,0)
        timeseries.append(t[:, :76])
    return torch.tensor(timeseries).float()

def get_pcorr(dataID):
    filenames = []
    for id in dataID.cpu().numpy():
        filenames.append(str(id)+".h5")
    pcorr = []
    for filename in filenames:
        temp = dd.io.load(osp.join("/data/CodeGoat24/data/ABIDE_pcp/cpac/filt_noglobal/raw", filename))
        # read edge and edge attribute
        pcorr = np.append(pcorr, np.abs(temp['pcorr'][()]))
        pcorr = torch.from_numpy(pcorr).float()
    return pcorr.reshape(len(filenames), 200, 200)

def compute_edge_info(pcorr):
    num_nodes = pcorr.shape[0]
    pcorr = pcorr.to_sparse()

    edge_att = pcorr.values()
    edge_index = torch.stack([pcorr.indices()[0], pcorr.indices()[1]])
    edge_index, edge_att = remove_self_loops(edge_index, edge_att)
    edge_index = edge_index.long()
    edge_index, edge_att = coalesce(edge_index, edge_att, num_nodes,
                                    num_nodes)

    return edge_att, edge_index, num_nodes


def get_edge_index_attr(pcorr):

    edge_att_list = []
    edge_index_list = []
    for j, pco in enumerate(pcorr):
        edge_att, edge_index, num_nodes = compute_edge_info(pco)
        edge_att_list.append(edge_att)
        edge_index_list.append(edge_index + j * num_nodes)

    edge_att_arr = torch.cat(edge_att_list)
    edge_index_arr = torch.cat(edge_index_list, axis=1)

    edge_att_torch = edge_att_arr.reshape(len(edge_att_arr), 1).float()
    edge_index_torch = edge_index_arr.long()

    return edge_att_torch, edge_index_torch
...
def get_ids(num_subjects=None):
    ...
    subject_IDs = np.genfromtxt(os.path.join(data_folder, 'subject_IDs.txt').replace('\\','/'), dtype=str)
    # subject_IDs = np.genfromtxt('./data/subject_ID.txt', dtype=str)
